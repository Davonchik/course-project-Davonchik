...
| NFR-03 | Срок жизни Access/Refresh  | Ограниченный TTL токенов                                                       | Access ≤ 5 мин; Refresh ≤ 30 мин    | Проверка через `settings` + тесты  | Tokens     | High      |
...
| NFR-05 | Авторизация по ролям       | Ограничение доступа к ресурсам по ролям                                        | user — только свои, admin — все     | Тесты `entries`                   | Entries    | High      |
